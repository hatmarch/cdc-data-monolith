...
mvn -B package -DskipTests \
        ...
        -f pass:[${DEMO_HOME}]/example/legacy-inventory-adaptor
----
...
[source,bash,subs="attributes+,+macros"]
----
oc apply -f pass:[${DEMO_HOME}]/example/legacy-inventory-adaptor/kube/legacy-consumer-adaptor.yaml -n pass:[${dev_prj}]
----
...
[source,bash,subs="attributes+,+macros"]
----
echo $(cat pass:[${DEMO_HOME}]/example/legacy-inventory-adaptor/src/test/resources/testCreateEvent.json) | oc exec -it demo-kafka-0 -n $dev_prj -- bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic pass:[${TOPIC}]
----